...
import { denyListedIPS, TESTS_ENABLED, Metadata } from './dclConfig'
...
export async function runChecks(
  ...
  const validCatalystPos: boolean = (TESTS_ENABLED && localhost.includes(metadata.realm.catalystName ?? ``))
    ? true
    : await checkPlayer(
      userAddress,
  ...
  const validPos: boolean = (TESTS_ENABLED && localhost.includes(metadata.realm.catalystName ?? ``)) 
    ? true : parcel?.length 
    ? checkCoords(coordinates!, parcel) : true
  if (!validPos) {
