    ...
    if (key === 'filters') {
      ...
      for (const operator in filters) {
        if (logicalOperators.includes(operator)) {
          ...
          if (isValid) {
            filter = { ...filter, ...unflattenObject({ [operator]: filters[operator] }) };
          } else {
...
export function parseQueryParams(queryParams: FilterQueryParams) {
  ...
  if (queryParams.sort) {
    parsedQueryParams.sort = queryParams.sort.split(',').reduce((acc: any, field: string) => {
...
export function buildMongoQuery(queryParams: FilterQueryParams) {
  ...
  if (parsedQueryParams.page && parsedQueryParams.limit) {
    mongoQuery.options.skip = (parsedQueryParams.page - 1) * parsedQueryParams.limit;
  }
