...
export async function runChecks(
	...
	const coordinates = metadata.parcel
		?.split(",")
		.map((item: string) => parseInt(item, 10));
	const localhost = [`127.0.0.1:8001`, `localhost`, `LocalPreview`];
	// check that the request comes from a decentraland domain
	const validOrigin =
		TESTS_ENABLED && localhost.includes(metadata.realm.catalystName ?? ``)
			? true
			: checkOrigin(req);
	if (!validOrigin) {
